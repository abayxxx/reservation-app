...
@section('title-1', 'Data Pertanyaan')
            ...
            <div class=" col-xl-12 mb-0 p-0">
                <div class="card-body">
                    <div class="card-block p-2 mt-3">
                        <button id="add" class="btn btn-outline-primary shadow-sm my-3"><i class="bi bi-plus"></i> Tambah Pertanyaan</button>
...
{{-- Modal --}}
@include('admin.pertanyaan.modal._modal')
@endsection
...
<script>
    $(document).ready(function() {
        ...
        $('#form-feature').on('submit', function(event) {
            ...
            if ($('#action').val() == 'edit') {
                url = "/admin/pertanyaan/update/" + id;
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        $('#kriteria_id').removeClass('is-invalid');
        ...
        $(document).on('click', '.edit', function() {
            let pertanyaanid = $(this).attr('id');
            $.ajax({
                url: '/admin/pertanyaan/' + pertanyaanid,
                dataType: 'JSON',
                success: function(data) {
                    $('#modal-title').html('Edit Pertanyaan')
                    $('#action').val('edit');
                    $('#kode_atribut').val(data.kode_atribut);
        ...
        $(document).on('click', '.delete', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        method: "DELETE",
                        url: '/admin/pertanyaan/' + pertanyaan_id,
                        headers: {
